...
class MarkdownFeature extends MarkdownDocumenterFeature {
  ...
  private saveSignatureInfo(
    ...
    const pathParts = eventArgs.outputFilename.split("/");
    const outputFilename = pathParts[pathParts.length - 1];

    if (!outputFilename.match(/[a-zA-Z-]+\.[_a-zA-Z0-9]+\.md/)) {
      return;
    }
  ...
  private async removeUnlessFile(): Promise<void> {
    ...
    const rejectFiles = files.filter(
      (file) => !file.match(/[a-zA-Z-]+\.[_a-zA-Z0-9]+\.md/),
    );
  ...
  private async cpDocs() {
    ...
    const mdPath = path.resolve(root, "../../generate-api-docs", "docs_md");
    const outPath = root;

    // Skip if docs_md folder doesn't exist
    if (!fs.existsSync(mdPath)) {
      console.log("MarkdownFeature: docs_md folder not found, skipping cpDocs");
      return;
    }
  ...
  private extractDescription(pageContent: string, displayName: string): string {
    ...
    for (const line of lines) {
      ...
      if (
        ...
        return desc || `FxTS ${displayName} - functional programming utility`;
      }
    }

    return `FxTS ${displayName} - functional programming utility for TypeScript/JavaScript`;
  }
