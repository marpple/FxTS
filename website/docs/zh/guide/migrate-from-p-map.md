...
相同的池语义应当意味着相同的实际耗时。以下是基于已发布包的实测(5 次运行取中位数,Node 24,`@fxts/core` 2.0.1 vs `p-map` 7):

| 场景                              | p-map | `concurrentPool` | `concurrent` |
| --------------------------------- | ----- | ---------------- | ------------ |
| 耗时不均(6 个任务,并发 2)         | 455ms | 455ms            | 654ms        |
| 均匀 50 × 50ms,并发 5             | 510ms | 511ms            | 511ms        |
| 5,000 个已 resolve 的任务,并发 10 | 4ms   | 9ms              | 2ms          |

迁移不是性能上的交换:在 I/O 密集型负载 — 并发限制器存在的根本原因 — 上,`concurrentPool` 与 p-map 持平。最后一行是坦诚的注脚 — 对 no-op 任务,池的保序簿记每项多花约 1 微秒,一旦任务做真实工作,这个差异就消失了。`concurrent` 列展示了上文所述的窗口行为。
