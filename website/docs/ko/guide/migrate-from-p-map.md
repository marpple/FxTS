...
같은 풀 의미론이라면 실제 소요 시간도 같아야 합니다. 배포된 패키지 기준 실측입니다(5회 실행 중앙값, Node 24, `@fxts/core` 2.0.1 vs `p-map` 7):

| 시나리오                               | p-map | `concurrentPool` | `concurrent` |
| -------------------------------------- | ----- | ---------------- | ------------ |
| 불균일 작업시간 (6개 작업, 동시성 2)   | 455ms | 455ms            | 654ms        |
| 균일 50개 × 50ms, 동시성 5             | 510ms | 511ms            | 511ms        |
| 이미 resolve된 작업 5,000개, 동시성 10 | 4ms   | 9ms              | 2ms          |

마이그레이션은 성능 트레이드가 아닙니다: 동시성 리미터가 존재하는 이유인 I/O 바운드 워크로드에서 `concurrentPool`은 p-map과 동일합니다. 마지막 행은 정직한 각주입니다 — no-op 작업에서는 풀의 순서 보존 장부 관리가 항목당 약 1마이크로초를 더 쓰는데, 작업이 실제 일을 하는 순간 사라지는 수준입니다. `concurrent` 열은 위에서 설명한 윈도우 동작을 보여줍니다.
