...
[pipe](/api/pipe)를 통해 Iterable/AsyncIterable을 처리할 수 있지만, `fxts`는 메서드 체이닝 형태로도 데이터 변환을 제공합니다.
...
지연 평가에 대한 자세한 내용은 /api/lazy-evaluation을 참조하세요.
...
`fxts`로 동시성을 처리하는 방법에 대한 자세한 내용은 /api/handle-concurrency를 참조하세요.
